import type { Collaborator, Document } from "@/types/document";
...
const mockDocuments: Document[] = [
	{
		...
		content:
			"<h1>Project Proposal</h1><p>This is a sample document with some content...</p>",
	...
	},
	{
		...
		title: "Meeting Notes",
		content:
			"<h1>Team Meeting - Jan 20, 2024</h1><p>Meeting agenda and notes...</p>",
...
export const documentService = {
	getUserDocuments: async (userId?: string): Promise<Document[]> => {
		...
		return mockDocuments.filter(
			(doc) =>
				doc.ownerId === userId ||
				doc.collaborators.some((c) => c.id === userId),
		);
	},
	...
	createDocument: async (data: {
		...
		const newDocument: Document = {
			...
			content:
				data.content ||
				`<h1>${data.title}</h1><p>Start writing your document here...</p>`,
			createdAt: new Date(),
			updatedAt: new Date(),
