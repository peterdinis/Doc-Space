  ...
  const handleDrop = (e: React.DragEvent, folderId: string) => {
    ...
    setFolders(prev => prev.map(folder => {
      if (folder.id === folderId) {
        const isAlreadyInFolder = folder.documents.some(doc => doc.id === draggedDocument.id);
        ...
        return {
          ...folder,
          documents: folder.documents.filter(doc => doc.id !== draggedDocument.id),
          documentsCount: Math.max(0, folder.documentsCount - (folder.documents.some(doc => doc.id === draggedDocument.id) ? 1 : 0))
        };
      }
      return folder;
    }));
    ...
    <Sidebar className="border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
      <SidebarHeader className="p-4 border-b border-gray-100 dark:border-gray-700">
        <div className="flex items-center space-x-3">
          ...
          {!isCollapsed && (
            <div>
              <h1 className="text-lg font-bold text-gray-900 dark:text-white">DocSpace</h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">Welcome, abcd</p>
            </div>
          )}
        </div>
      </SidebarHeader>
      ...
      <SidebarContent className="p-2">
              ...
              <SidebarMenu>
                {filteredDocuments.slice(0, 5).map((document) => (
                  <SidebarMenuItem key={document.id}>
                    ...
                    >
                      <Link href={`/document/${document.id}`} className="flex items-center justify-between w-full">
        ...
        <SidebarGroup>
          ...
          >
            ...
            {!isCollapsed && (
              <Button variant="ghost" size="sm" className="h-6 w-6 p-0 hover:bg-gray-200 dark:hover:bg-gray-600">
                    ...
                    >
                      ...
                      {!isCollapsed && (
                        <>
                          ...
                          <span className="text-xs text-gray-400 dark:text-gray-500">{folder.documentsCount}</span>
                        </>
                      )}
                    </SidebarMenuButton>
                    ...
                    <SidebarMenuButton className="transition-colors duration-200 hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      {!isCollapsed && (
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{connection.name}</div>
                          <div className="text-xs text-gray-500 dark:text-gray-400 truncate">{connection.email}</div>
                        </div>
                      )}
                    </SidebarMenuButton>
                  </SidebarMenuItem>
